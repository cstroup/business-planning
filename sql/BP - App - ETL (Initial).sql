...
-- SELECT * FROM [dbo].[expense_classification]
UPDATE [dbo].[expense_classification]
SET [expense_classification] = 'Capex - BAU'
WHERE [raw] IN ('Capex - BAU  Capex - BAU', 'Capex- BAU')
;

UPDATE [dbo].[expense_classification]
SET [expense_classification] = 'Capex - BAU'
WHERE LOWER([raw]) LIKE 'capex%'
AND LOWER([raw]) LIKE '%bau%'
;

UPDATE [dbo].[expense_classification]
SET [expense_classification] = 'Opex - BAU'
WHERE [raw] IN ('Opexx - BAU')
;
...
-- SELECT * FROM [dbo].[segmentation] order by 1

UPDATE [dbo].[segmentation]
SET [segmentation] = 'Data & Intelligence'
WHERE [raw] IN ('Data and Intelligence')
;

UPDATE [dbo].[segmentation]
SET [segmentation] = 'Data Science Experimentation'
WHERE [raw] IN ('Data Science   Experimentation')
;

UPDATE [dbo].[segmentation]
SET [segmentation] = 'Labs & Environments'
WHERE [raw] IN ('Labs and Environments')
;

UPDATE [dbo].[segmentation]
SET [segmentation] = 'Systems & Licensing'
WHERE [raw] IN ('Systems and Licensing')
;
...
WHERE [raw] IN ('Active- Pending Conversion')
;
...
-- update the manual forecast items from the old gl
DROP TABLE IF EXISTS #TMP_OLD_GL_FORECAST_MANUAL;
SELECT 
	sap_gl.[JL ID],
	sap_gl.[PO Composite],
	CAST(LEFT(sap_gl.[JL ID], CHARINDEX('.', sap_gl.[JL ID]) - 1) as BIGINT) as je,
	CAST(RIGHT(sap_gl.[JL ID], LEN(sap_gl.[JL ID]) - CHARINDEX('.', sap_gl.[JL ID])) AS BIGINT) as je_li,
	sap_gl.[FCSTID], 
	f.[old_forecast_id],
	f.[forecast_id]
INTO #TMP_OLD_GL_FORECAST_MANUAL
FROM [Compiler].[mart].[SAP GL] as sap_gl
JOIN [dbo].[forecast] as f
	ON sap_gl.[FCSTID] = f.[old_forecast_id]
AND f.[is_deleted] = 0
;
...
FROM #TMP_OLD_GL_FORECAST_MANUAL as t, [dbo].[general_ledger] 
WHERE t.je = [general_ledger].[journal_entry]
	AND t.je_li = [general_ledger].[journal_entry_item]
;



-- figure out how to map missing departments
DROP TABLE IF EXISTS #TMP_UPDATE_DEPARTMENT;
SELECT
	f.[forecast_id],
	f.[department_id],
	gl.[department_id] as new_department_id,
	RANK() OVER (PARTITION BY f.[forecast_id] ORDER BY f.[forecast_id]) AS dedupe
INTO #TMP_UPDATE_DEPARTMENT
FROM [dbo].[forecast] as f
JOIN (SELECT DISTINCT [department_id], [forecast_id] FROM [dbo].[general_ledger] WHERE [forecast_id] IS NOT NULL) as gl
	ON f.[forecast_id] = gl.forecast_id
WHERE f.[department_id] = 0
ORDER BY 1 DESC
;


UPDATE [dbo].[forecast]
SET [forecast].[department_id] = t.[new_department_id]
FROM #TMP_UPDATE_DEPARTMENT as t, [dbo].[forecast] 
WHERE t.[forecast_id] = [forecast].[forecast_id]
;


---- figure out how to map missing departments
--DROP TABLE IF EXISTS #TMP_UPDATE_DEPARTMENT_V2;
--SELECT
--	f.[forecast_id],
--	f.[department_id],
--	gl.[department_id] as new_department_id,
--	RANK() OVER (PARTITION BY f.[forecast_id] ORDER BY f.[forecast_id]) AS dedupe
---- INTO #TMP_UPDATE_DEPARTMENT_V2
--FROM [dbo].[forecast] as f
--JOIN (
--	SELECT DISTINCT 
--		[auto_tag_id], 
--		[forecast_id]
--	FROM [dbo].[auto_tag] WHERE is_deleted = 0
--	) as tag
--	ON f.[forecast_id] = tag.[forecast_id]
--JOIN (SELECT DISTINCT [department_id], [forecast_id] FROM [dbo].[general_ledger] WHERE [forecast_id] IS NOT NULL) as gl
--	ON tag.[forecast_id] = gl.forecast_id
----WHERE f.[department_id] = 0
--ORDER BY 1 DESC
--;
