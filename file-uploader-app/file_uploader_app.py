...
python -m PyInstaller -–onefile -–windowed --name="Reporting File Uploader" --hidden-import openpyxl file_uploader_app.py
"""
...
class FileUploader:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title('Reporting File Uploader')
    ...
    def create_admin_portal(self):
        ...
        tk.Label(server_frame, text="Server Name:", anchor='w').pack(side='left', padx=5, pady=5, anchor='w')
        server_options = ["business-planning-proxy.spectrumtoolbox.com", "VM0PWDCRPTD0001"]
        self.server_var = tk.StringVar()
        self.server_var.set(server_options[0])
        self.server_option = ttk.Combobox(server_frame, textvariable=self.server_var, values=server_options, width=50, height=20)
        ...
        tk.Label(db_frame, text="Database Name:", anchor='w').pack(side='left', padx=5, pady=5, anchor='w')
        db_options = ["Compiler", "EID", "PLANNING_APP", "REPORTING", "TEST"]
        ...
        username_frame.pack(side='top', fill='both', padx=5, pady=5, expand=False)
        tk.Label(username_frame, text="Username:", anchor='w').pack(side='left', padx=5, pady=5, anchor='w')
        self.username_entry = tk.Entry(username_frame, width=30)
        self.username_entry.pack(side='left', padx=5, pady=5, anchor='w')

        # Password
        pwd_frame = tk.Frame(self.root)
        pwd_frame.pack(side='top', fill='both', padx=5, pady=5, expand=False)
        tk.Label(pwd_frame, text="Password:", anchor='w').pack(side='left', padx=5, pady=5, anchor='w')
        self.password_entry = tk.Entry(pwd_frame, show="*", width=30)
        self.password_entry.pack(side='left', padx=5, pady=5, anchor='w')

        # Add a separator
        # ttk.Separator(self.frame, orient='horizontal').pack(fill='x', padx=20, pady=20)

        # Upload Files
        self.upload_frame = tk.Frame(self.root)
        self.upload_frame.pack(side='top', fill='both', padx=5, pady=25, expand=False)
        self.upload_button = ttk.Button(self.upload_frame, text="Upload Files", 
                                        command=self.upload_files,
                                        style="Important.TButton")
        self.upload_button.pack(side='left', padx=5, pady=5, anchor='w')
        self.upload_label = tk.Label(self.upload_frame, text="", anchor='w')
        self.upload_label.pack(side='left', padx=5, pady=5, anchor='w')

        # Allocation Timesheets File
        timesheets_frame = tk.Frame(self.root)
        timesheets_frame.pack(side='top', fill='both', padx=5, pady=5, expand=False)
        tk.Label(timesheets_frame, text="Allocation Timesheets File").pack(side='left', padx=5, pady=5, anchor='w')
        self.select_timesheets_button = ttk.Button(timesheets_frame, text="Browse CSV", 
                                                command=self.select_timesheets_file,
                                                style="TButton")
        self.select_timesheets_button.pack(side='left', padx=5, pady=5, anchor='w')
        self.timesheets_label = tk.Label(timesheets_frame, text=" ", anchor='w', width=70)
        self.timesheets_label.pack(side='left', padx=5, pady=5, anchor='w')

        # Contractor Details File
        contractor_frame = tk.Frame(self.root)
        contractor_frame.pack(side='top', fill='both', padx=5, pady=5, expand=False)
        tk.Label(contractor_frame, text="Contractor Details File").pack(side='left', padx=5, pady=5, anchor='w')
        self.select_contractors_button = ttk.Button(contractor_frame, text="Browse Excel", 
                                                command=self.select_contractor_details,
                                                style="TButton")
        self.select_contractors_button.pack(side='left', padx=5, pady=5, anchor='w')
        self.contractors_label = tk.Label(contractor_frame, text=" ", anchor='w', width=70)
        self.contractors_label.pack(side='left', padx=5, pady=5, anchor='w')

        # Keach HR File
        hr_frame = tk.Frame(self.root)
        hr_frame.pack(side='top', fill='both', padx=5, pady=5, expand=False)
        tk.Label(hr_frame, text="Keach HR File").pack(side='left', padx=5, pady=5, anchor='w')
        self.select_hr_button = ttk.Button(hr_frame, text="Browse Excel", 
                                                command=self.select_keach_hr,
                                                style="TButton")
        self.select_hr_button.pack(side='left', padx=5, pady=5, anchor='w')
        self.keach_hr_label = tk.Label(hr_frame, text=" ", anchor='w', width=70)
        self.keach_hr_label.pack(side='left', padx=5, pady=5, anchor='w')
    ...
    def insert_dataframe_to_sql_server(self, df, schema, table, batch_size, truncate_table='N'):
        ...
        # get database name from drop-down
        self.database_name = self.db_option.get()
        ...
        if truncate_table == 'Y':
            ...
            self.upload_label.config(text=f"{truncate_query}", width=100)
            self.root.update()
        else:
            pass
        ...
        for i in range(0, df_len, batch_size):
            ...
            self.upload_label.config(text=f"{info_label}", width=100)
            self.root.update()
    ...
    def excel_file_to_dataframe(self, directory, skiprows=0, skipfooter=0, display_df='N'):
        ...
        try:
            ...
            msg = str(e)
            print(f'Failed: {msg}')
            tk.messagebox.showerror("Error", f"There was an error with the upload! {msg}") 
    ...
    def upload_files(self):
        ...
        try:
            ...
            if self.timesheets_file_path and os.path.isfile(self.timesheets_file_path) and len(self.timesheets_file_path) > 0:
            ...
            if self.contractor_details_file_path and os.path.isfile(self.contractor_details_file_path) and len(self.contractor_details_file_path) > 0:
            ...
            if self.keach_hr_file_path and os.path.isfile(self.keach_hr_file_path) and len(self.keach_hr_file_path) > 0:
            ...
            self.upload_label.config(text=f"Completed!", width=100)
            self.root.update()
